...
export default { async fetch(request, env) {
  ...
  if (!contract) return json({}, env, request, 503, { ok: false, error: "sync_not_configured" });
  const url = new URL(request.url);

  if (request.method === "OPTIONS" && url.pathname === contract.publicWorker.chatApi) {
    const h = securityHeaders();
    corsHeaders(contract, env, request).forEach((v, k) => h.set(k, v));
    return new Response(null, { status: 204, headers: h });
  }

  if (url.pathname === contract.publicWorker.healthApi || url.pathname === "/") {
    return json(contract, env, request, 200, { ok: true, worker: PUBLIC_NAME, api: contract.publicWorker.chatApi, repoSync: contract.repo.repoSync, assetId: contract.repo.assetId, ioProConfigured: Boolean(safeText(env.IO_PRO || "", 1)) });
  }

  if (url.pathname !== contract.publicWorker.chatApi) return json(contract, env, request, 404, { ok: false, error: "not_found" });
  if (request.method !== "POST") return json(contract, env, request, 405, { ok: false, error: "method_not_allowed" });

  const origin = normalizeOrigin(request.headers.get("Origin") || "");
  const allowed = allowedOrigins(contract, env);
  if (!origin) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  if (!allowed.has(origin)) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  if (!safeText(env.IO_PRO || "", 2048)) return json(contract, env, request, 503, { ok: false, error: "sync_not_configured" });
  ...
  if (clientName !== safeText(contract.repo.clientName, 64)) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  if (repoSync !== safeText(contract.repo.repoSync, 64)) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  if (assetId !== safeText(contract.repo.assetId, 120)) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  if (!sessionId || !headerIntegrity) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  ...
  if (!raw || raw.length > maxBody) return json(contract, env, request, 400, { ok: false, error: "request_blocked" });

  let body;
  try { body = JSON.parse(raw); } catch { return json(contract, env, request, 400, { ok: false, error: "request_blocked" }); }
  const required = contract.repoRequestBody?.requiredFields || [];
  if (!required.every((key) => Object.prototype.hasOwnProperty.call(body || {}, key))) return json(contract, env, request, 400, { ok: false, error: "request_blocked" });

  const honeypot = safeText(body.honeypot || body.website || "", 300);
  if (honeypot) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  ...
  if (!cleanMessage || risk.score >= maxRisk) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  ...
  if (!clientIntegrity || !timingSafeEq(clientIntegrity, serverIntegrity)) return json(contract, env, request, 403, { ok: false, error: "request_blocked" });
  ...
  let upstream; try { upstream = await forwardToBinding(env, clean); } catch { return json(contract, env, request, 502, { ok: false, error: "request_blocked" }); }
  if (!upstream.ok) return json(contract, env, request, 502, { ok: false, error: "request_blocked" });
  ...
  return json(contract, env, request, 200, { ok: true, reply, integrity: serverIntegrity }, { "x-gabo-chatbot-gateway": "1", "x-gabo-integrity-verified": "1", "x-gabo-repo-sync-verified": "1" });
} };
