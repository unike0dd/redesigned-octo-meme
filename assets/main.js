(function () {
  ...
  const HONEYPOT_SESSION_BLOCK_KEY = "gabo-contact-honeypot-blocked";
  ...
  function getHoneypotFields(form) {
    return Array.from(form.querySelectorAll("[data-honeypot='true']"));
  }

  function isHoneypotTriggered(form) {
    return getHoneypotFields(form).some((field) =>
      String(field.value || "").trim(),
    );
  }

  function closeContactSecuritySession(form, statusNode) {
    form.dataset.securitySessionBlocked = "true";
    try {
      window.sessionStorage?.setItem(HONEYPOT_SESSION_BLOCK_KEY, "true");
    } catch {
      // If storage is unavailable, the in-memory form lock still blocks forwarding.
    }
    ...
    if (statusNode) {
      statusNode.textContent =
        "This contact session was closed by the TinyML gateway integrity check.";
    }
  }

  function isContactSecuritySessionClosed() {
    try {
      return (
        window.sessionStorage?.getItem(HONEYPOT_SESSION_BLOCK_KEY) === "true"
      );
    } catch {
      return false;
    }
  ...
  async function secureFormSubmission(form, statusNode) {
    ...
    ).filter((field) => !field.disabled && field.dataset.honeypot !== "true");
  ...
  function initSecureForms() {
    ...
    forms.forEach((form) => {
      ...
      if (isContactSecuritySessionClosed()) {
        closeContactSecuritySession(form, message);
        return;
      }
      ...
      form.addEventListener("submit", async (event) => {
        event.preventDefault();
        if (form.dataset.securitySessionBlocked === "true") return;
        if (isHoneypotTriggered(form)) {
          closeContactSecuritySession(form, message);
          return;
        }
