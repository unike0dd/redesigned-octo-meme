...
The HTML form is fail-closed: its native `action` points back to the page, while `tiny-ml.js` reads the Cloudflare Worker URL from `data-cf-worker-url` only after the Contact-specific scanner has passed and replaced form values with cleansed data.
