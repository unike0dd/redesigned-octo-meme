...
function parseJsonArray(input, fallback = []) {
  try {
    const parsed = JSON.parse(toStr(input || ""));
    if (Array.isArray(parsed)) return parsed;
  } catch {}
  return fallback;
}

function loadAllowedOrigins(env) { const fallback = ["https://www.gabo.services", "https://gabo.services"]; const parsed = parseJsonArray(env.ALLOWED_ORIGINS_JSON, fallback); return new Set(parsed.map(normalizeOrigin).filter(Boolean)); }
function securityHeaders(extra = {}) { const h = new Headers(extra); h.set("Content-Security-Policy", CSP); h.set("Strict-Transport-Security", "max-age=63072000; includeSubDomains; preload"); h.set("X-Content-Type-Options", "nosniff"); h.set("X-Frame-Options", "DENY"); h.set("Referrer-Policy", "strict-origin-when-cross-origin"); h.set("Cross-Origin-Opener-Policy", "same-origin"); h.set("Cross-Origin-Resource-Policy", "same-origin"); h.set("Permissions-Policy", STANDARD_PERMISSIONS_POLICY); h.set("X-Permitted-Cross-Domain-Policies", "none"); h.set("X-DNS-Prefetch-Control", "off"); h.set("X-Robots-Tag", "noindex, nofollow"); h.set("Cache-Control", "no-store, no-transform"); return h; }
function corsHeaders(env, request) { const allowed = loadAllowedOrigins(env); const origin = normalizeOrigin(request.headers.get("Origin") || ""); const h = new Headers(); if (origin && allowed.has(origin)) { h.set("Access-Control-Allow-Origin", origin); h.set("Vary", "Origin"); } const methods = parseJsonArray(env.ALLOWED_METHODS_JSON, ["POST", "OPTIONS"]).map((x) => safeText(x, 20).toUpperCase()).filter(Boolean); const allowHeaders = parseJsonArray(env.ALLOWED_HEADERS_JSON, ["content-type", "accept", HEADER_INTEGRITY, HEADER_CLIENT, HEADER_SESSION]).map((x) => safeText(x, 80).toLowerCase()).filter(Boolean); const exposeHeaders = parseJsonArray(env.EXPOSE_HEADERS_JSON, ["x-gabo-chatbot-gateway", "x-gabo-integrity-verified"]).map((x) => safeText(x, 80).toLowerCase()).filter(Boolean); h.set("Access-Control-Allow-Methods", methods.join(", ") || "POST, OPTIONS"); h.set("Access-Control-Allow-Headers", allowHeaders.join(", ") || ["content-type", "accept", HEADER_INTEGRITY, HEADER_CLIENT, HEADER_SESSION].join(", ")); h.set("Access-Control-Expose-Headers", exposeHeaders.join(", ") || "x-gabo-chatbot-gateway, x-gabo-integrity-verified"); h.set("Access-Control-Max-Age", "86400"); return h; }
