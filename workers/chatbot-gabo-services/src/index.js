...
function loadAllowedOrigins(env) { const fallback = ["https://www.gabo.services", "https://gabo.services", "https://unike0dd.github.io"]; try { const parsed = JSON.parse(toStr(env.ALLOWED_ORIGINS_JSON || "")); if (Array.isArray(parsed) && parsed.length) return new Set(parsed.map(normalizeOrigin).filter(Boolean)); } catch {} return new Set(fallback.map(normalizeOrigin)); }
